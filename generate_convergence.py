...
RMSE_FBP_vec = np.zeros((len(data), ))
RMSE_vec = np.zeros((len(data), ))
for i in range(len(data)):
    ...
    RMSE_FBP_vec[i] = metrics.RMSE(x_fbp, x_true[0])
    RMSE_vec[i] = metrics.RMSE(x_tpv, x_true[0])

    # Verbose
    print(f"{i+1}/{len(data)}. FBP RMSE: {RMSE_FBP_vec[i]}, TpV RMSE: {RMSE_vec[i]}.")

# Save the metrics
RMSE_path = f'./results/metrics/{args.data}/{args.mode}'

utilities.create_path_if_not_exists(RMSE_path)

np.save(f"{RMSE_path}/RMSE_FBP_{args.experiment}.npy", RMSE_FBP_vec)
np.save(f"{RMSE_path}/RMSE_IS_{args.experiment}_p_{args.p}.npy", RMSE_vec)